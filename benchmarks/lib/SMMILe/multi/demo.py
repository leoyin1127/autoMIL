...
def summary(model, data, patch_size, sp_data,n_classes, inst_refinement):
    ...
    with torch.no_grad():
        ...
        _, Y_prob, Y_hat, ref_score, results_dict = model(feaetures_nic, mask, sp, adj,label=np.array([]), 
...
def main_eval(npy_path,sp_path, model_type, model_size, n_classes, fea_dim,patch_size, n_refs, ckpt_path, inst_refinement):
    ...
    results_dict, df_inst = summary(model, data, patch_size,sp_data,n_classes, inst_refinement)
