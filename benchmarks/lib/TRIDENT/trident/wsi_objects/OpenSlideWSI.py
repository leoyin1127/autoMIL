...
class OpenSlideWSI(WSI):
    ...
    def __getstate__(self):
        """Drop unpicklable OpenSlide objects for DataLoader workers.

        ``self.img`` is the OpenSlide handle (ctypes pointers).
        ``self.properties`` is an ``openslide._PropertyMap`` that also
        wraps a ctypes pointer internally.  Both must be excluded or
        converted before pickling.
        """
        state = self.__dict__.copy()
        state.pop('img', None)
        # _PropertyMap wraps a ctypes pointer; snapshot to a plain dict
        if 'properties' in state and state['properties'] is not None:
            state['properties'] = dict(state['properties'])
        return state

    def __setstate__(self, state):
        """Re-open the OpenSlide handle in the worker process."""
