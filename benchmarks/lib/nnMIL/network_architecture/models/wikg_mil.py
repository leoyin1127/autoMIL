...
class WIKG_MIL(nn.Module):
    ...
    def forward(self, x, return_WSI_attn = False, return_WSI_feature = False):
        ...
        # Dynamically adjust topk to not exceed sequence length
        B, N, _ = e_h.shape
        actual_topk = min(self.topk, N)
        topk_weight, topk_index = torch.topk(attn_logit, k=actual_topk, dim=-1)
        ...
        e_h_expand = e_h.unsqueeze(2).expand(-1, -1, actual_topk, -1)
        ...
        B, N, C = h.shape
        h_flat = h.view(-1, C)  # [B*N, C]
        
        # Create batch indices for readout
        batch_idx = torch.arange(B, device=h.device).repeat_interleave(N)
        
        # Apply readout with proper batch handling
        h_pooled = self.readout(h_flat, batch=batch_idx)  # [B, C]
        h_pooled = self.norm(h_pooled)
        logits = self.fc(h_pooled)  # [B, num_classes]
        forward_return['logits'] = logits
        if return_WSI_feature:
            forward_return['WSI_feature'] = h_pooled
        if return_WSI_attn:
